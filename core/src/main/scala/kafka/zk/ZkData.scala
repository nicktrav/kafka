...
object ControllerZNode {
  ...
  def encode(brokerId: Int, timestamp: Long): Array[Byte] =
    Json.legacyEncodeAsBytes(Map("version" -> 1, "brokerid" -> brokerId, "timestamp" -> timestamp.toString))
...
object TopicZNode {
  ...
  def encode(assignment: collection.Map[TopicPartition, Seq[Int]]): Array[Byte] = {
    ...
    Json.legacyEncodeAsBytes(Map("version" -> 1, "partitions" -> assignmentJson))
  }
...
object TopicPartitionStateZNode {
  ...
  def encode(leaderIsrAndControllerEpoch: LeaderIsrAndControllerEpoch): Array[Byte] = {
    ...
    Json.legacyEncodeAsBytes(Map("version" -> 1, "leader" -> leaderAndIsr.leader, "leader_epoch" -> leaderAndIsr.leaderEpoch,
      "controller_epoch" -> controllerEpoch, "isr" -> leaderAndIsr.isr))
  }
...
object ConfigEntityZNode {
  ...
  def encode(config: Properties): Array[Byte] = {
    import scala.collection.JavaConverters._
    Json.legacyEncodeAsBytes(Map("version" -> 1, "config" -> config.asScala))
  }
...
object ConfigEntityChangeNotificationSequenceZNode {
  ...
  def encode(sanitizedEntityPath : String): Array[Byte] = Json.legacyEncodeAsBytes(Map("version" -> 2, "entity_path" -> sanitizedEntityPath))
...
object IsrChangeNotificationSequenceZNode {
  ...
  def encode(partitions: Set[TopicPartition]): Array[Byte] = {
    ...
    Json.legacyEncodeAsBytes(Map("version" -> IsrChangeNotificationHandler.Version, "partitions" -> partitionsJson))
  }
...
object LogDirEventNotificationSequenceZNode {
  ...
  def encode(brokerId: Int) =
    Json.legacyEncodeAsBytes(Map("version" -> 1, "broker" -> brokerId, "event" -> LogDirFailureEvent))
  def decode(bytes: Array[Byte]): Option[Int] = Json.parseBytes(bytes).map { js =>
...
object ReassignPartitionsZNode {
  ...
  case class ReplicaAssignment(@JsonProperty("topic") topic: String,
                               @JsonProperty("partition") partition: Int,
  ...
  case class PartitionAssignment(@JsonProperty("version") version: Int,
                                 @JsonProperty("partitions") partitions: java.util.Collection[ReplicaAssignment])
...
object PreferredReplicaElectionZNode {
  ...
  def encode(partitions: Set[TopicPartition]): Array[Byte] = {
    ...
    Json.legacyEncodeAsBytes(jsonMap)
  }
...
object ResourceZNode {
  ...
  def encode(acls: Set[Acl]): Array[Byte] = Json.legacyEncodeAsBytes(Acl.toJsonCompatibleMap(acls))
